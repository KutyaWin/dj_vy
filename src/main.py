...
async def run_site_crawl_demo() -> dict[str, object]:
    start_urls = ["https://example.com"]
    crawler = AsyncCrawler(max_concurrent=5, per_domain_concurrent=2, max_depth=1, html_parser=HTMLParser())
    crawl_result: dict[str, object] = {}
    elapsed = 0.0
    output_dir = Path(tempfile.gettempdir()) / "dj_vy_site_crawl_demo"
    output_dir.mkdir(parents=True, exist_ok=True)
...
def print_site_crawl_demo_summary(result: dict[str, object]) -> None:
    print_section("SITE CRAWL DEMONSTRATION")
    print(f"Start URLs: {', '.join(list(result['start_urls']))}")
    print(f"Output JSON: {result['output_path']}")
    print(f"Elapsed: {float(result['elapsed']):.2f}s")
    stats = dict(result["stats"])
    print(f"Processed pages: {stats.get('processed_pages', 0)}")
    print(f"Queued pages: {stats.get('pending', 0)}")
    print(f"Failed pages: {stats.get('failed_pages', 0)}")
    print(f"Speed: {float(stats.get('pages_per_second', 0.0)):.2f} pages/sec")
    for url, page in dict(result["processed_urls"]).items():
        print(
            f"- url={url} | depth={page.get('depth', 0)} | title={page.get('title', '')} | "
            f"links={len(list(page.get('links', [])))}"
        )
